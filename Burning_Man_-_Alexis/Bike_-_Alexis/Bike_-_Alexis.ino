...
void lightningSpark() {
  ...
  ls_numberOfFlashes = random8(1, 6); // Determine number of flashes
  ...
  for (int x = 0; x <= ls_numberOfFlashes; x++)
  {
    ...
    ls_flashBrightness = random(BRIGHTNESS / 10 * x, BRIGHTNESS);
...
void lightningLine() {
  ...
  if (lightningStrand < 6) {
    ...
    for (int z = 0; z <= ll_numberOfFlashes; z++) {
      ...
      ll_flashBrightness = random(BRIGHTNESS / 10 * z, BRIGHTNESS); // Flash the white line, brighter
